	...
	<div class="info-container" ref="infoBox" :class="containerClasses">
		...
		<div class="info-buttons">
			...
			<div class="tutorial-buttons" v-if="status == 'tutorial'">
				<Button class="btn-tutorial" @click="resetToLegends">Skip</Button>
				<div class="btn-group">
					<Button class="btn-tutorial" :disabled="tutorialIndex == 0" @click="tutorialIndex--">Prev</Button>
					<Button class="btn-tutorial" @click="tutorialIndex == tutorial.length-1 ? resetToLegends() : tutorialIndex++">{{ tutorialIndex == tutorial.length-1 ? 'End' : 'Next' }}</Button>
				</div>
...
.info-container {
	...
	padding: 25px;
	border-radius: 5px;
	background: linear-gradient(0, #390099 0%, #000000 240%);
	color: white;
